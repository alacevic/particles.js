...
var pJS = function(tag_id, params){
  ...
  pJS.fn.particlesCreate = function(){
    ...
    pJS.fn.setParticleTargetShape(0);
  ...
  pJS.fn.setParticleTargetShape = function(shapeIndex){
    ...
    // set the destination: 
    //----------------------------------------------
    //var res = pJS.targetShape;
    var pointsRelative = true;

    for (var i = 0, iParticle = 0; i < res.length && pJS.particles.array.length > iParticle; i++)
    {
      //get a particle
      var p = pJS.particles.array[iParticle];
      
      //parse point from res
      var pointxy = res[i].split(",");

      //add new point to our array:
      if (pointxy.length == 2)
      {
        pointxy[0] = parseFloat(pointxy[0]) * scaleFactor;
        pointxy[1] = parseFloat(pointxy[1]) * scaleFactor;
        
        var prevPoint = {"destX": 0, "destY": 0};
        if (iParticle > 0 && pointsRelative == true)
          prevPoint = pJS.particles.array[iParticle - 1];
        
        //formShape[formShape.length] = {"x":pointxy[0] + prevPoint.x, "y":pointxy[1] + prevPoint.y, "linkToNext": true};
        if (prevPoint != null && p != null){
          p.destX = pointxy[0] + prevPoint.destX;
          p.destY = pointxy[1] + prevPoint.destY;
          p.linkToNext = true;
        }
      ...
      {
        if (iParticle > 0 && iParticle <= pJS.particles.array.length)
          pJS.particles.array[iParticle - 1].linkToNext = false;
      }
      else if (pointxy[0] == 'm'){
        pointsRelative = true;
      }
      else if (pointxy[0] == 'M'){
        pointsRelative = false;
      }
    ...
    for(var i = 0; i < pJS.particles.array.length; i++){
      ...
      if (pJS.particles.move.enable){
        ...
        if (p.destX != null) {
          ...
          //otherwise keep the particle moving in the direction set randomly
          p.x += p.vx * ms;
          p.y += p.vy * ms;
        }
      }
